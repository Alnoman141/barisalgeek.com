                    ...
                    <div class="container">
                        ...
                        <form action="{{ route('admin.student.update',$student->id) }}" method="post" enctype="multipart/form-data">
                                ...
                                <div class="col-sm-10">
                                    <select class="custom-select" name="batch_id">
                                        ...
                                        @foreach(App\Models\Batch::orderBy('id','desc')->get() as $batch)
                                            <option {{ $student->batch_id == $batch->id?'selected':'' }} value="{{ $batch->id }}">{{ $batch->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <img src="{{ asset('images/student/'.$student->image) }}" alt="BarisalGeek Student {{ $student->last_name }}" height="150px">
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <button type="submit" class="btn btn-info btn-sm"  >Update Students</button>
                                </div>
                            </div>
                        </form>
                    </div>
